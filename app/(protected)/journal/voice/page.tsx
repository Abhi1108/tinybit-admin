...
import { Mic, Loader2, AlertCircle, Search, ExternalLink } from 'lucide-react';
import { getAdminJournalEntries, type JournalEntryRecord } from '@/src/services/adminApi';

export default function VoiceJournalsPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setError(null);
      try {
        const res = await getAdminJournalEntries({ limit: 100, type: 'Voice' });
        if (res.success) setRows(res.entries || []);
        else setError(res.error || 'Failed to load voice journals');
      } catch (err) {
        setError(err instanceof Error ? err.message : 'Failed to load voice journals');
      } finally {
        setLoading(false);
      }
    }
  ...
  return (
    <div className="space-y-6">
      <div className="page-header">
        <div>
          <h1 className="page-title flex items-center gap-2"><Mic className="w-6 h-6 text-brand-500" /> Voice Journals</h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-0.5">Voice memory journal entries</p>
        </div>
      </div>
      ...
      <div className="card overflow-hidden">
        {loading ? (
          ...
          <div className="divide-y divide-slate-100 dark:divide-slate-800">
              ...
              <div key={row.id} className="px-5 py-4">
                ...
                >
                  ...
                  <p className="text-sm text-slate-600 dark:text-slate-400 line-clamp-2">
                    {expanded === row.id ? row.content : (row.content_preview || row.content || 'Voice entry')}
                  </p>
                </button>
                {row.audio_uri && (
                  <a
            ...
            {filtered.length === 0 && <div className="py-16 text-center text-slate-400 text-sm">No voice journals found</div>}
          </div>
        )}
      </div>
    </div>
